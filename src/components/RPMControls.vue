...
const props = withDefaults(defineProps<{ RPM: number }>(), {
  RPM: 0
})
...
const handleIncreaseRPM = function () {
  ...
  // Cap at 2100
  const RPM = Math.min(Math.ceil((props.RPM + 100) / 100) * 100, 2100)
...
const handleDecreaseRPM = function () {
  ...
  // Cap at 700
  const RPM = Math.max(Math.ceil((props.RPM - 100) / 100) * 100, 700)
  console.log(props.RPM, RPM)
...
<template>
  <div class="RPM__controls">
    <Up :class="{ disabled: disableUp }" @click="handleIncreaseRPM" />
    <Down :class="{ disabled: disableDown }" @click="handleDecreaseRPM" />
  </div>
</template>
