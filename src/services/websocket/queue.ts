...
export const addToQueue = function addToQueue(
  command: IWebSocketCommand,
  connectionId: number,
  send: boolean = true
) {
...
export const sendQueuedCommands = function sendQueuedCommands() {
  ...
  const command = WebsocketQueue.find((command: IWebSocketCommand) => command.status === 'new')
...
export const handleResponseMessage = function handleResponseMessage(event: MessageEvent) {
  ...
  const command = WebsocketQueue.find((command: IWebSocketCommand) => command.id === message.id)
